...
export default {
    ...
    updateWorkspaceInfo(workspacePath) {
        ...
        if (this.envsh) {
            atom.notifications.addError(`Autoproj: already updating workspace information`)
            return;
        }
        this.envsh = child_process.spawn(autoprojExePath, ['envsh'], {cwd: workspacePath, stdio: 'ignore'})

        let busySignal = `Autoproj: Update Workspace Info of ${workspacePath}`
        this.busyProvider && this.busyProvider.add(busySignal);
        this.envsh.on('close', (code) => {
            this.envsh = undefined;
            if (code != 0) {
                atom.notifications.addError(`Autoproj: Failed to update workspace information for ${workspacePath}`)
            }
            this.busyProvider && this.busyProvider.remove(busySignal, code == 0);
        })
        return this.envsh
    },
