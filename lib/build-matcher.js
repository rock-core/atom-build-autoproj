...
const orogenRubyError = /^([^:]+\.orogen):(\d+):(?:\d*:)?(.*)$/
...
const orogenErrorMatch = (lines) => {
    ...
    lines.forEach((line) => {
        ...
        match = orogenRubyError.exec(line)
        if (match) {
            result.push({ file: match[1], line: match[2], type: 'Error', message: match[3] })
            return;
        }
...
exports.plainBuildMatcher = (packageDir, packageType, mode, output) => {
    var matcher = findLineMatcher(packageType, mode)
    if (matcher) {
        var matches = matcher(output.split(/\r?\n/))
        normalizeEntries(packageDir, matches)
        return matches
    }
    else {
        return []
...
const normalizeEntries = (dir, entries) => {
    entries.forEach((entry) => {
        ...
        entry.file = FS.realpathSync(entry.file);
        var trace = entry.trace;
        if (trace) {
            normalizeEntries(dir, trace)
        }
    })
}
...
exports.autoprojBuildMatcher = (workspaceInfo, output) => {
    ...
    perPackageAndMatcher.forEach((perMatcher, packageName) => {
        var packageDir = workspaceInfo.packages.get(packageName).srcdir
        perMatcher.forEach((output, matcher) => {
            var matches = matcher(output)
            normalizeEntries(packageDir, matches)
