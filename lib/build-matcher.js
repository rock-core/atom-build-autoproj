...
const errorAndWarningMatch = (errorMatchers, warningMatchers, lines) => {
    ...
    lines.forEach((line) => {
        ...
        if (match) {
            result.push({ file: match.file, line: match.line, column: match.column, type: 'Error', message: match.message })
            return
        }
        ...
        if (match) {
            result.push({ file: match.file, line: match.line, column: match.column, type: 'Warning', message: match.message })
            return
        }
    })
    return result
